...
class DatabaseService {
  ...
  static const int _databaseVersion = 4; // เวอร์ชันของฐานข้อมูล
  ...
  Future<void> _onCreate(Database db, int version) async {
    ...
    await db.execute('''
      CREATE TABLE $eventsTable (
        ...
        is_recurring INTEGER DEFAULT 0,        -- สถานะการทำซ้ำ (0=false, 1=true)
        recurring_weekdays TEXT,               -- วันในสัปดาห์ที่ทำซ้ำ (คั่นด้วยจุลภาค)
        deadline_date TEXT,                    -- วันที่สิ้นสุดการทำซ้ำ
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    // ถ้าเวอร์ชันเก่าน้อยกว่า 4
    if (oldVersion < 4) {
      // ✅ เพิ่มฟิลด์สำหรับการทำซ้ำในตาราง events
      await db.execute(
        'ALTER TABLE $eventsTable ADD COLUMN is_recurring INTEGER DEFAULT 0',
      );
      await db.execute(
        'ALTER TABLE $eventsTable ADD COLUMN recurring_weekdays TEXT',
      );
      await db.execute(
        'ALTER TABLE $eventsTable ADD COLUMN deadline_date TEXT',
      );
    }
  }
